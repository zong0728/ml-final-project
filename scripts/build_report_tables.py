...
"""
    ...
    body = (
        ...
        "greedy allocation of the 5 backup generators. Given each generator's "
        r"\textbf{1{,}000-household cap}, the marginal benefit of placing one in "
        "county $c$ at decision step $s$ is the integral of the predicted "
        "outage in $c$ over the 48-hour horizon, capped both at 1{,}000 (per "
        "generator) and at the county's tracked household maximum:\n"
        + eq_block +
        "\nWe greedily pick the county maximizing $M_s$, deduct the served "
        r"capacity, and repeat for $s{=}1,\dots,5$. Multiple generators "
        r"\emph{may} go to the same county if its 48h served-household-hours "
        "still exceed 1{,}000 after the previous draw. Our recommendation is:"
        f"\n\n\\begin{{quote}}\n\\verb|{sel}|\n\\end{{quote}}\n\n"
